...
class ExtractMetaGraphIntegrationTest : AbstractJvmCircuitTest()  {
	...
	private fun extract(): MetaGraph {
		...
		val uuid = GraphViewModule.metaGraphService.extractMetaGraph("Extract", drawingView, componentIds, library)
	...
	fun shouldReplaceSwitchesAndLEDs() {
		...
		assertEquals(2, targetMetaGraph.graph.graphView.getDrawableInstances<CircuitInOutView>().size)
	}
}