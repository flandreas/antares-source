...
class FSMTruthTableCreatorTest {
    ...
    fun shouldCreateTruthTable() {
        val fsm = createFSMDrawing(useNames = true)
    ...
    @Test
    fun shouldCreateTruthTableWithUnnamedExpressions() {
        val fsm = createFSMDrawing(useNames = false)
        ...
        // Expect that columns receive default input and output names
        assertEquals("O", truthTable.getColumnName(5))
        assertEquals("Z_0^(n+1)", truthTable.getColumnName(4))
        assertEquals("Z_1^(n+1)", truthTable.getColumnName(3))
        assertEquals("I", truthTable.getColumnName(2))
    ...
    private fun createFSMDrawing(useNames: Boolean): FSMDrawing {
        val fsm = FSMDrawing()
