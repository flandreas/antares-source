...
class RectangularComponentTest {
    ...
    fun shouldReturnBounds() {
        val rect = RectangularComponent(styleProvider = StyleProviderMockBuilder().build(), bounds = Rectangle2D(10, 20, 30, 40))
        assertThat(rect.bounds.x, `is`(10.0))
        assertThat(rect.bounds.y, `is`(20.0))
        assertThat(rect.bounds.width, `is`(30.0))
        assertThat(rect.bounds.height, `is`(40.0))
    }
