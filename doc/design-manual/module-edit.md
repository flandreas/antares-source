...
A `Component` is an editable and `Drawable` object and the central element of the `edit` module. It implements `Storable` to be written to and read from external storage, and has an `id` to support references. It is by default `Movable` and `Rotatable`, and consumes its visual properties as `Styleable` from its `Style`.
...
A `DrawingView` extends `draw.View` by not only displaying the current `Drawing`, but also additional "layers" needed to implement other visual parts of a 2D graphics editor. These layers or `Containers` include (from top-most to bottom-most):

- a "Ghost" container used for temporarily drawing `Unzoomable` objects like `Handles` of `SelectionModels`
...
`Editor` implements support for editing a `Drawing` within a `DrawingView`. It provides `Tools` like the `SelectionTool` the user can use to select `Components`. It maintains a `CommandManager` that helps make changes to the `Drawing` undoable and redoable. Its `SnapManager` and `DragManager` assist the user when moving `Components` within the `Drawing`.   

## Selection handling

The Jabbah framework contains very versatile structures for making `Components` selectable. A `SelectionModel` not only draws a `Component` in "selected" state, it can also offer graphical hints like `Handles` the user can interact with to change the shape of a `Component`. The framework supports 3 different `SelectionDrawingStrategies` "Above", "Replace" and "Below", which indicate where a `SelectionModel` is drawn relative to its `Component`.
...
For example, see `LEDViewBeanInfo` and how it defines an editable property for an `LEDView`'s `LightColor` color property, whose property editor is registered in `AntaresModuleJvm` as `LightColorEditor`, which displays a combobox with all predefined `LightColors`.
...
Snapping is the process of forcing mouse coordinates to certain restricted locations, either defined by a `Grid`, or by already existing `Components` that implement the `Snappable` interface. A `SnapManager` held by the `Editor` mediates coordinate restrictions requested by possibly many `Snappers`, like when both a `Grid` and `ComponentSnapper` is active.
