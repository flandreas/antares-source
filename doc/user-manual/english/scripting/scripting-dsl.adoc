...
As in any other programming language, you can use named variables to store values. Variable names must start with a character. Use the assignment operator "=" to assign a value to a variable.
...
Variable declaration is optional in _Antares DSL_. If you assign a value to a variable that hasn't been used yet, it gets implicitly declared. However, you can use the "var" keyword to declare a variable explicitly. This is particularly useful within a block of code.
...
* Inputs usually come before outputs
* In logic gates, it's the inverse: The output has ID 1, the inputs start with ID 2 (which is the topmost input if the gate's output is facing east)
====
