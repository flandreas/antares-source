...
New signals always arrive at the right edge of each signal row, and then move towards the left edge when time passes, and eventually leave the visible area. The oscilloscope allows to scroll the visible area horizontally by clicking on a signal row and dragging horizontally.
