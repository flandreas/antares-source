...
If the simulation is put into "Single-step mode" (menu "Simulation") before starting to execute a use case, the simulation is halted in a breakpoint before execution of each line of the execution stript. The execution can then be resumed by pressing the pause/resume button.

Note that breaking at "each line" is only possible if the individual lines are all timed differently. If you have 3 lines in your script, and each of them has the same execution time, they are all executed at the same time, and you will only experience a single breakpoint.

Tip:: Turn off "menu:Simulation[Soft Breakpoints]" when stepping through use case execution script lines. Soft breakpoints will halt the simulation with every changing pin output, which will distract you from focusing on the use case actions.
