...
image:../../img/base-library/switch-sample.png[Beispiel Schalter, 400]

==== Verhalten

Diese Komponente implementiert einen Schalter, den der Benutzer mit der linken Maustaste drücken kann. Ein gedrückter Schalter erzeugt am Ausgang den Wert 1, ansonsten ist der Wert 0. Je nach Wert des Attributs "Umschalten" bleibt der Schalter nach Loslassen der Maustaste eingeschaltet, oder er schaltet sich wieder aus.

Beim Drücken des Schalters mit der Maus erhält der Schalter den Tastatur-Fokus, der durch ein dünnes blaues gestricheltes Rechteck dargestellt wird. Wenn ein Schalter den Tastatur-Fokus besitzt, kann er auch mit der kbd:[Enter]-Taste sowie mit den Tasten kbd:[0] und kbd:[1] betätigt werden.

Ein gedrückter Schalter erzeugt nicht sofort ein Signal am Ausgang, sondern erst nach Ablauf der eingestellten Laufzeit, die standardmässig auf 1'000 ns eingestellt ist. Bis diese Laufzeit verstrichen ist, nimmt der Schalter keine weiteren Eingaben entgegen. Dies wird ersichtlich, wenn die Simulation im Einzelschritt-Modus ausgeführt wird: Der Schalter wird bis zum Ablauf der Laufzeit als "inaktiv" (mit grauer Fläche überlagert) dargestellt.
