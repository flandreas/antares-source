...
Der Symboleditor besteht aus einem Baum, der die Elemente anzeigt, die in das Symbol hineingezogen werden können, und der grossen Zeichnungsfläche, in der das Symbol gezeichnet wird. Zusätzlich können die Zeichnungswerkzeuge verwendet werden, um graphische Elemente wie Rechtecke zum Symbol hinzuzufügen.
...
Anzahl Bits anzeigen:: Anschlüsse mit einer Bit-Breite grösser als 1 stellten standardmässig eine kleine Annotation dar, welche die Anzahl Bits des Anschlusses anzeigt. Mit diesem Attribut kann diese Darstellung ausgeblendet werden.
...
Darstellung während Simulation:: In diesem Attribut kann ein Skript eingegeben werden, mit dem die Darstellung des Symbols (in engen Grenzen) während der Simulation zusätzlich beeinflusst werden kann. Beispielsweise enthalten die Multiplexer-Komponenten der Standard-Bibliothek von Antares ein Skript, das den ausgewählten Datenpfad innerhalb des Symbols dargestellt wird, indem eine Linie vom Eingang zum Ausgang gezeichnet wird. +
 +
image:../../img/subcircuits/exec-draw-sample.png[, 350]
 +
Skripting ist ein vorgeschrittenes Konzept. Siehe das Kapitel <<../scripting/scripting.adoc#, "Scripting">> für weitere Informationen.
...
Steuerelemnte sind Komponenten der Schaltung, die zusätzlich in das Symbol aufgenommen werden und während der Simulation ihren aktuellen Zustand im Symbol darstellen. Interaktive Komponenten wie Schalter ermöglichen es zusätzlich, das der Benützer während der Simulation Aktionen wie das Drücken des Schalters ausführt, die dann in der Teilschaltung verarbeitet werden.
...
Als weiteres Beispiel, das die Möglichkeiten von Steuerelementen veranschaulicht, ist die folgende Tastatur-Schaltung. Sie enthält 9 Schalter, die in drei Spalten und drei Zeilen angeordnet sind. Die Schaltung gibt die Adresse der gedrückten Schalters über die jeweiligen Ausgänge Cx und Rx aus.
...
Beachte, wie der Benützer die Tastatur-Schaltung während der Simulation wie eine ausprogrammierte Basis-Komponente verwenden kann. Sie zeigt ihre Schalter mit ihren aktuellen Zustand an und erlaubt es es dem Benutzer, die Schalter mit der Maus zu klicken.

Wenn man diese Anwendungsidee noch weiter denkt, kann man sich vorstellen, in einem Projekt eine spezielle Schaltung aufzunehmen, deren Symbol alle interaktiven Elemente sämtlicher Teilschaltungen enthält. Damit kann diese spezielle Schaltung wie als "Front Panel" eines komplexen Schaltung eingesetzt werden; öffnet man diese Schaltung, sieht man nur noch die interaktiven Elemente des Systems, also die Schalter, LEDs oder 7-Segment-Anzeigen.
