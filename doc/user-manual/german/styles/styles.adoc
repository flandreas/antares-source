...
Das wichtigste Beispiel der Bündelung von grafischen Eigenschaften in Antares ist die **Farbe**. Die Farbe "Grün" ist in Antares nicht einfach nur ein bestimmter RGB-Wert wie z.B. "(0,255,0)", sondern sie besteht aus drei unterschiedlichen RGB-Werten für Vordergrundfarbe, Hintergrundfarbe und Textfarbe.  In Programmcode von Antares wird eine solche Farbe als "Zusammengesetzte Farbe (Composite Color)" bezeichnet.
...
NOTE: Das Beispiel zeigt die Attribute eines Rechtecks. Bei anderen Komponenten kann das Eigenschaftenfenster anders aussehen.
...
NOTE: Das Attribut "Schatten" hat nur eine Wirkung, wenn die Darstellung von Schatten in den Einstellungen von Antares grundsätzlich aktivert ist.
