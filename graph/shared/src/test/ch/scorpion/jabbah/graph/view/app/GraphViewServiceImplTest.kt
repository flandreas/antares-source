...
class GraphViewServiceImplTest {
	...
	fun shouldAddWrapperWhenAddingNonGraphElementView() {
		service.add(RectangleComponent(), DrawingViewMockBuilder().withDrawing(testGraphView.graphView).build())

		assertTrue(testGraphView.graphView.get(0) is GraphElementViewWrapper<*>)
	}
