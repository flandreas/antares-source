...
class UsecaseTestRunner(
	...
	val script = Script(usecase.testScript!!, usecase.name.value, Translations.getString("usecaseTest.issueContext.name"))
	...
	fun assert(time: Long, condition: () -> Boolean, description: String? = null) {
		...
		scheduler.requestActingAfter(UsecaseTestActor(condition), delay(time), SimpleActorData(description))
	}
	...
	private inner class UsecaseTestActor(private val condition: () -> Boolean) : EmptyActor() {
		override fun act(signalHandler: SignalHandler, data: ActorData): Boolean {
			if (!condition.invoke()) {
				LOG.debug("Test '${usecase.name.value}' failed")
				postTestFailure(data.dataToString())
	...
	private fun postTestFailure(description: String?) {
