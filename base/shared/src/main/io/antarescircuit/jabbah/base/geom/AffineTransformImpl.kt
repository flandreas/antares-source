...
data class AffineTransformImpl(
	...
	@Suppress("unused")
	constructor(t: AffineTransformImpl) : this(t.m00, t.m10, t.m01, t.m11, t.m02, t.m12)
	...
	companion object {
		...
		const val APPLY_IDENTITY = 0
		const val APPLY_TRANSLATE = 1
		const val APPLY_SCALE = 2
		const val APPLY_SHEAR = 4

		const val HI_SHIFT = 3
		const val HI_IDENTITY = APPLY_IDENTITY shl HI_SHIFT
		const val HI_TRANSLATE = APPLY_TRANSLATE shl HI_SHIFT
		const val HI_SCALE = APPLY_SCALE shl HI_SHIFT
		const val HI_SHEAR = APPLY_SHEAR shl HI_SHIFT
	...
	override fun concatenate(Tx: AffineTransform) {
		...
		if (Tx !is AffineTransformImpl) {
			throw IllegalArgumentException("unsupported ${Tx::class.simpleName}")
		}
