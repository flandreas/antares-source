...
import io.antarescircuit.jabbah.base.swing.StateBar
...
class BusyHandler(val rootPaneContainer: RootPaneContainer, val stateBar: StateBar?) {
	...
	companion object {
		...
		val LOG by logger(Companion::class)

		/** Stores all registered [BusyHandler]s.*/
		private val HANDLERS: MutableMap<RootPaneContainer, BusyHandler> = mutableMapOf()

		/** Used for determining the top-most registered [RootPaneContainer].*/
		private val STACK = Stack<BusyHandler>()
		...
		fun register(busyHandler: BusyHandler): BusyHandler {
			synchronized(HANDLERS) {
				...
				HANDLERS[busyHandler.rootPaneContainer] = busyHandler
				STACK.push(busyHandler)
			}
			return busyHandler
		}
		...
		fun topLevel(): RootPaneContainer? = STACK.optionalPeek()?.rootPaneContainer
