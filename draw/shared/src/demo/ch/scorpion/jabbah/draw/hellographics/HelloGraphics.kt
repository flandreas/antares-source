...
class SimpleRectangle(bounds: Rectangle2D, private val fillColor: Color) : AbstractRectangle(bounds), Locatable {
	...
	@Suppress("unused")
	constructor(bounds: Rectangle2D) : this(bounds, Color(255, 255, 255))
...
class Model(
	...
	private fun addContent(rectCount: Int) {
		for (i in 1..rectCount) {
			val width = Random.nextDouble(MIN_WIDTH, MAX_WIDTH)
			val height = Random.nextDouble(MIN_HEIGHT, MAX_HEIGHT)
			val centerX = Random.nextDouble(width / 2, dim.width - width / 2)
			val centerY = Random.nextDouble(height / 2, dim.height - height / 2)

			val color = Color(
				Random.nextDouble(0.0, 255.0).toInt(),
				Random.nextDouble(0.0, 255.0).toInt(),
				Random.nextDouble(0.0, 255.0).toInt(),
				Random.nextDouble(0.0, 255.0).toInt())
